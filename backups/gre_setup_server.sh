date > ../timestamp-records/gre.server.sh.start_ts
...
echo 111
echo sudo ovs-vsctl add-port $router_bridge_name $router_port_name -- set interface $router_port_name type=vxlan, options:remote_ip=$router_ip
...
echo 222
echo router_port=`sudo ovs-vsctl -- --columns=name,ofport list Interface $router_port_name| tail -n1| egrep -o "[0-9]+"`
router_port=`sudo ovs-vsctl -- --columns=name,ofport list Interface $router_port_name| tail -n1| egrep -o "[0-9]+"`
echo 333
echo sudo ifconfig $router_bridge_name $router_anycast_ip/32 up
...
date > ../timestamp-records/gre.server.sh.end_ts