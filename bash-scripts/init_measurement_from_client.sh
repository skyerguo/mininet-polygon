...
do
    ...
    ## 每3000+毫秒记录一次，计算平均的数据按最近15秒算，按照kBit/s记录结果

    ## 刷新间隔，稍微错开一点，防止nload同时跑起来
    refresh_interval=`awk 'BEGIN{print "3000" + ("50" * "'$i'")}'`
    # echo "refresh_interval: " $refresh_interval
    sudo nload -a 15 -u k -t $refresh_interval -m devices c${client_id}-eth$i > ${measurement_result_path}nload/nload_log_c${client_id}_cz${client_zone}_s${i}.txt &
