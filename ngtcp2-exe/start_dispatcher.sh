...
do 
    {
        ...
        echo sudo LD_LIBRARY_PATH=/data /data/dispatcher --datacenter $dispatcher_id --user johnson --password johnson "d"${dispatcher_id}"-eth1" 0.0.0.0 $port /data/server.key /data/server.crt --current_dispatcher_name "d"$dispatcher_id -q >> ${output_file}_tmp.txt

        sudo LD_LIBRARY_PATH=/data /data/dispatcher --datacenter $dispatcher_id --user johnson --password johnson "d"${dispatcher_id}"-eth1" 0.0.0.0 $port /data/server.key /data/server.crt --current_dispatcher_name "d"$dispatcher_id -q 1>> ${output_file}_1.txt 2>> ${output_file}_2.txt
